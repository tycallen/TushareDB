...
def load_daily_data(start_date='20200101', end_date='20260130', sample_stocks=500):
    """加载日线数据并计算前复权价格（采样部分股票以加快处理）"""
    conn = get_connection()
...
# ============================================================================
# 突破类型识别模块 (优化版 - 使用向量化操作)
# ============================================================================

def detect_new_high_breakout_vectorized(df, period=60):
    """
    检测新高突破 (向量化版本)
    """
    print(f"      正在计算{period}日新高突破...")
    results = []
...
def detect_range_breakout_vectorized(df, period=20, threshold=0.08):
    """
    检测区间突破 (向量化版本)
    """
    print(f"      正在计算区间突破...")
    ...
    for ts_code, group in df.groupby('ts_code'):
        ...
        # 筛选震荡区间且突破
        mask = (group['range_pct'] < threshold) & (group['close'] > group['range_high'])
        breakouts = group[mask].copy()
...
def detect_ma_breakout_vectorized(df, ma_periods=[20, 60, 120]):
    """
    检测均线突破 (向量化版本)
    """
    print(f"      正在计算均线突破...")
    ...
    for ts_code, group in df.groupby('ts_code'):
        ...
        for ma_period in ma_periods:
            ...
            # 检测向上突破均线
            mask = (group['close_prev'] < group[f'ma{ma_period}_prev']) & (group['close'] > group[f'ma{ma_period}'])
            breakouts = group[mask].copy()
...
def detect_volume_breakout_vectorized(df, vol_period=20, vol_multiplier=2.0):
    """
    检测成交量突破 (向量化版本)
    """
    print(f"      正在计算成交量突破...")
    ...
    for ts_code, group in df.groupby('ts_code'):
        ...
        # 检测放量上涨
        mask = (group['vol_ratio'] > vol_multiplier) & (group['pct_chg'] > 0)
        breakouts = group[mask].copy()
...
def calculate_post_breakout_returns(df, breakouts, holding_periods=[5, 10, 20, 60], max_samples=2000):
    """计算突破后收益 (采样)"""
    print(f"      正在计算突破后收益（样本数: {min(len(breakouts), max_samples)}）...")
    ...
    for ts_code in breakouts['ts_code'].unique():
        ...
        for _, row in stock_breakouts.iterrows():
            ...
            if idx + max(holding_periods) >= len(stock_data):
                continue
            ...
            for period in holding_periods:
                if idx + period < len(stock_data):
                    exit_price = stock_data.iloc[idx + period]['close']
                    period_data = stock_data.iloc[idx:idx + period + 1]
...
def identify_true_false_breakout(df, breakouts, confirmation_days=3, threshold=0.02, max_samples=2000):
    """识别真假突破 (采样)"""
    print(f"      正在识别真假突破...")
    ...
    for ts_code in breakouts['ts_code'].unique():
        ...
        for _, row in stock_breakouts.iterrows():
            ...
            if idx + confirmation_days >= len(stock_data):
                continue

            # 检查确认期内的最低价
            confirm_data = stock_data.iloc[idx + 1:idx + confirmation_days + 1]
            min_low = confirm_data['low'].min()

            # 判断是否为假突破
            is_true_breakout = min_low >= breakout_price * (1 - threshold)
...
def analyze_breakout_effectiveness(validation_df, return_df):
    """分析突破有效性"""
    if validation_df.empty or return_df.empty:
        return {}

    # 合并数据
    merged = validation_df.merge(return_df, on=['ts_code', 'trade_date', 'breakout_type'], how='inner')
...
def backtest_breakout_strategy(df, breakouts, stop_loss=0.05, take_profit=0.15,
                               max_hold_days=20, max_samples=2000):
    """回测突破买入策略"""
    if breakouts.empty:
        return pd.DataFrame()
    ...
    for ts_code in breakouts['ts_code'].unique():
        ...
        for _, row in stock_breakouts.iterrows():
            ...
            if idx + max_hold_days >= len(stock_data):
                continue
            ...
            for i in range(1, min(max_hold_days + 1, len(stock_data) - idx)):
                day_data = stock_data.iloc[idx + i]
            ...
            if exit_date is None:
                exit_date = stock_data.iloc[idx + max_hold_days]['trade_date']
                exit_price = stock_data.iloc[idx + max_hold_days]['close']
...
def backtest_pullback_strategy(df, breakouts, pullback_ratio=0.03, confirm_ratio=0.02,
                                stop_loss=0.05, take_profit=0.15, max_hold_days=20, max_samples=1500):
    """回测回踩确认策略"""
    if breakouts.empty:
        return pd.DataFrame()
    ...
    for ts_code in breakouts['ts_code'].unique():
        ...
        for _, row in stock_breakouts.iterrows():
            ...
            breakout_idx = date_to_idx[breakout_date]
            breakout_price = row['close']

            if breakout_idx + 30 >= len(stock_data):
                continue
            ...
            for i in range(1, min(11, len(stock_data) - breakout_idx)):
                day_data = stock_data.iloc[breakout_idx + i]
            ...
            if entry_idx + max_hold_days >= len(stock_data):
                continue
            ...
            for i in range(1, min(max_hold_days + 1, len(stock_data) - entry_idx)):
                day_data = stock_data.iloc[entry_idx + i]
            ...
            if exit_date is None:
                exit_date = stock_data.iloc[entry_idx + max_hold_days]['trade_date']
                exit_price = stock_data.iloc[entry_idx + max_hold_days]['close']
                exit_reason = 'max_hold'

            trade = {
                'ts_code': ts_code,
                'breakout_date': breakout_date,
                'entry_date': stock_data.iloc[entry_idx]['trade_date'],
                'entry_price': entry_price,
                'exit_date': exit_date,
                'exit_price': exit_price,
                'exit_reason': exit_reason,
...
def generate_report(df, breakout_stats, validation_results, strategy_results):
    """生成研究报告"""
...
- 样本股票：{df['ts_code'].nunique()}只（随机采样）
- 数据记录：{len(df):,}条
...
**信号统计**
- 60日新高突破信号总数：{breakout_stats.get('new_high_count', 0):,}
- 平均每只股票产生信号：{breakout_stats.get('new_high_per_stock', 0):.1f}次
...
**信号统计**
- 区间突破信号总数：{breakout_stats.get('range_count', 0):,}
- 区间震幅阈值：8%
...
**信号统计**
- 均线突破信号总数：{breakout_stats.get('ma_count', 0):,}
- 分布情况：
  - MA20突破：{breakout_stats.get('ma20_count', 0):,}
  - MA60突破：{breakout_stats.get('ma60_count', 0):,}
  - MA120突破：{breakout_stats.get('ma120_count', 0):,}
...
**信号统计**
- 成交量突破信号总数：{breakout_stats.get('volume_count', 0):,}
...
**60日新高突破验证结果**
{format_validation_text(validation_results.get('new_high', {}))}

### 2.2 突破后走势统计

**60日新高突破后收益分布**
{format_returns_text(validation_results.get('new_high_returns', pd.DataFrame()))}

**MA60均线突破后收益分布**
{format_returns_text(validation_results.get('ma60_returns', pd.DataFrame()))}
...
{format_strategy_table(strategy_results)}
...
{format_pullback_results(strategy_results)}
...
def format_returns_text(returns_df):
    """格式化收益统计文本"""
    if returns_df.empty:
        return "（数据不足，无法计算）"
...
"""
    ...
    for key, name in strategy_names.items():
        if key in strategy_results:
            ...
            text += f"| {name} | {r['total_trades']} | {r['win_rate']:.1f}% | {r['avg_return']:.2f}% | {r['avg_win']:.2f}% | {r['avg_loss']:.2f}% | {pf_str} |\n"
...
def main():
    ...
    # 加载数据
    print("\n[1] 加载数据...")
    df = load_daily_data(start_date='20200101', end_date='20251231', sample_stocks=500)
    ...
    # 1.1 新高突破
    print("\n[1.1] 检测60日新高突破...")
    new_high_breakouts = detect_new_high_breakout_vectorized(df, period=60)
    print(f"      检测到突破信号: {len(new_high_breakouts):,}")

    # 1.2 区间突破
    print("\n[1.2] 检测区间突破...")
    range_breakouts = detect_range_breakout_vectorized(df, period=20, threshold=0.08)
    print(f"      检测到突破信号: {len(range_breakouts):,}")

    # 1.3 均线突破
    print("\n[1.3] 检测均线突破...")
    ma_breakouts = detect_ma_breakout_vectorized(df, ma_periods=[20, 60, 120])
    ...
    # 1.4 成交量突破
    print("\n[1.4] 检测成交量突破...")
    volume_breakouts = detect_volume_breakout_vectorized(df, vol_period=20, vol_multiplier=2.0)
    print(f"      检测到突破信号: {len(volume_breakouts):,}")

    # 统计突破信息
    breakout_stats = {
        'new_high_count': len(new_high_breakouts),
        'new_high_per_stock': len(new_high_breakouts) / df['ts_code'].nunique() if df['ts_code'].nunique() > 0 else 0,
    ...
    if not new_high_breakouts.empty:
        new_high_returns = calculate_post_breakout_returns(df, new_high_breakouts, max_samples=2000)
        new_high_validation = identify_true_false_breakout(df, new_high_breakouts, max_samples=2000)
        new_high_analysis = analyze_breakout_effectiveness(new_high_validation, new_high_returns)

        validation_results['new_high'] = new_high_analysis
        validation_results['new_high_returns'] = new_high_returns

        print(f"      样本数量: {len(new_high_validation):,}")
    ...
    print("\n[2.2] MA60均线突破有效性验证...")
    if not ma_breakouts.empty:
        ma60_breakouts = ma_breakouts[ma_breakouts['breakout_period'] == 60]
        if not ma60_breakouts.empty:
            ma60_returns = calculate_post_breakout_returns(df, ma60_breakouts, max_samples=2000)
            ma60_validation = identify_true_false_breakout(df, ma60_breakouts, max_samples=2000)
            ma60_analysis = analyze_breakout_effectiveness(ma60_validation, ma60_returns)

            validation_results['ma60'] = ma60_analysis
            validation_results['ma60_returns'] = ma60_returns

            print(f"      MA60样本数量: {len(ma60_validation):,}")
            if ma60_analysis:
    ...
    print("      - 60日新高突破策略...")
    if not new_high_breakouts.empty:
        new_high_trades = backtest_breakout_strategy(df, new_high_breakouts,
                                                      stop_loss=0.05, take_profit=0.15, max_hold_days=20)
        if not new_high_trades.empty:
            new_high_eval = evaluate_strategy(new_high_trades)
    ...
    if not ma_breakouts.empty:
        ma60_breakouts = ma_breakouts[ma_breakouts['breakout_period'] == 60]
        if not ma60_breakouts.empty:
            ma60_trades = backtest_breakout_strategy(df, ma60_breakouts,
                                                      stop_loss=0.05, take_profit=0.15, max_hold_days=20)
            if not ma60_trades.empty:
                ma60_eval = evaluate_strategy(ma60_trades)
    ...
    # 区间突破策略
    print("      - 区间突破策略...")
    if not range_breakouts.empty:
        range_trades = backtest_breakout_strategy(df, range_breakouts,
                                                   stop_loss=0.05, take_profit=0.15, max_hold_days=20)
        if not range_trades.empty:
            range_eval = evaluate_strategy(range_trades)
    ...
    print("      - 60日新高回踩确认策略...")
    if not new_high_breakouts.empty:
        pullback_trades = backtest_pullback_strategy(df, new_high_breakouts,
                                                      pullback_ratio=0.03, confirm_ratio=0.02,
                                                      stop_loss=0.05, take_profit=0.15, max_hold_days=20)
        if not pullback_trades.empty:
            pullback_eval = evaluate_strategy(pullback_trades)
    ...
    report = generate_report(df, breakout_stats, validation_results, strategy_results)
